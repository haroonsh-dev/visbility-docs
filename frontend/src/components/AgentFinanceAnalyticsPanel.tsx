...
import {
    ...
    deriveFinViewKpis,
    extractOverdueQueue,
    extractVendorRegister,
    finGroupForView,
    type FinAnalyticsGroupId,
...
export default function AgentFinanceAnalyticsPanel({
    accent,
    snapshot,
    visuals,
    loading,
    ...
    const [group, setGroup] = useState<FinAnalyticsGroupId>(() => finGroupForView(view));

    const activeGroup = FIN_ANALYTICS_GROUPS.find((g) => g.id === group) || FIN_ANALYTICS_GROUPS[0];
    const kpis = useMemo(() => deriveFinViewKpis(visuals, view), [visuals, view]);
    const register = useMemo(() => extractVendorRegister(visuals, 10), [visuals]);
    const overdue = useMemo(() => extractOverdueQueue(visuals, 5), [visuals]);
    const showRegister =
        register.length > 0 && (view === "vendors" || view === "overview" || group === "ap");
    const showOverdue = overdue.length > 0 && (view === "aging" || group === "ap");

    const selectGroup = (id: FinAnalyticsGroupId) => {
        setGroup(id);
        const g = FIN_ANALYTICS_GROUPS.find((x) => x.id === id);
        ...
        <div className="flex flex-col rounded-2xl border border-border bg-surface/30 overflow-hidden">
            <div className="shrink-0 border-b border-border bg-surface/40 px-4 sm:px-5 py-4 space-y-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <p className="text-sm font-bold text-foreground">Finance analytics</p>
                        <p className="text-xs text-foreground-muted mt-0.5">
                            {snapshot.stats.totalFiles} files · {snapshot.stats.vendorCount} vendors · {visuals.length}{" "}
                            chart{visuals.length === 1 ? "" : "s"}
                        </p>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                        {FIN_ANALYTICS_GROUPS.map((g) => (
                            <button
                                key={g.id}
                                type="button"
            ...
            {showOverdue && (
                <div className="shrink-0 px-4 sm:px-5 py-3 border-b border-border">
                    <AgentFinanceRegisterTable
                        ...
                        accent={accent}
                        maxRows={5}
                        onAsk={() => onRunPrompt("Chart AP aging and summarize overdue vendors")}
                        compact
                        title="Overdue AP buckets"
                    />
                </div>
            )}
