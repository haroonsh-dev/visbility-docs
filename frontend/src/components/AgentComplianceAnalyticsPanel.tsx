...
export default function AgentComplianceAnalyticsPanel({
    accent,
    snapshot,
    visuals,
    loading,
    ...
    const [group, setGroup] = useState<CompAnalyticsGroupId>(() => compGroupForView(view));

    const activeGroup = COMP_ANALYTICS_GROUPS.find((g) => g.id === group) || COMP_ANALYTICS_GROUPS[0];
    const kpis = useMemo(() => deriveCompViewKpis(visuals, view), [visuals, view]);
    const register = useMemo(() => extractCertRegister(visuals, 10), [visuals]);
    const showRegister =
        register.length > 0 &&
        (view === "expiry" || view === "cert_status" || view === "overview" || group === "certs");

    const selectGroup = (id: CompAnalyticsGroupId) => {
        setGroup(id);
        const g = COMP_ANALYTICS_GROUPS.find((x) => x.id === id);
        ...
        <div className="min-h-[520px] flex flex-col rounded-2xl border border-border bg-surface/30 overflow-hidden">
            <div className="shrink-0 border-b border-border bg-surface/40 px-4 sm:px-5 py-4 space-y-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <p className="text-sm font-bold text-foreground">Compliance analytics</p>
                        <p className="text-xs text-foreground-muted mt-0.5">
                            {snapshot.stats.totalFiles} files · {snapshot.stats.certsTracked} certs · {visuals.length}{" "}
                            chart{visuals.length === 1 ? "" : "s"}
                        </p>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                        {COMP_ANALYTICS_GROUPS.map((g) => (
                            <button
                                key={g.id}
                                type="button"
