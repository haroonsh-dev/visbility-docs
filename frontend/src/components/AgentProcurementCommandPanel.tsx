...
export default function AgentProcurementCommandPanel({
    snapshot,
    accent,
    visuals,
    attention,
    ...
    const register = extractOrderRegister(visuals, 8);

    const fmtSpend =
        snapshot.stats.totalSpend > 0
            ? `${snapshot.stats.currency} ${Math.round(snapshot.stats.totalSpend).toLocaleString()}`
            : "—";
    ...
    return (
        <div className="space-y-5">
            ...
            >
                ...
                {!isEmpty && (
                    <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-border/50">
                        {[
                            { label: "Committed spend", value: fmtSpend },
                            { label: "Open POs", value: snapshot.stats.openCount || "—" },
                            { label: "Fulfilled", value: snapshot.stats.fulfilledCount || "—" },
                            {
                                label: "Needs review",
                                value: snapshot.stats.discrepancyCount || "—",
                            },
                        ].map((s) => (
                            <div key={s.label}>
                ...
                {isEmpty && (
                    ...
                    >
                        <Upload size={15} /> Upload procurement documents
                    </Link>
                )}
            </div>
            ...
            {register.length > 0 && (
                <AgentProcurementRegisterTable rows={register} accent={accent} onAsk={onAsk} />
            )}
