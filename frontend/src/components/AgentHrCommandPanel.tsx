...
import { extractCvShortlist, type HrPillar, type HrPriority, type HrWorkforceSnapshot } from "@/lib/agentWorkspaceHr";
...
function PillarCard({
    ...
    return (
        <button
            ...
            className="group text-left rounded-2xl border border-border bg-surface/40 p-4 hover:border-border hover:bg-surface/70 transition-all"
        >
            ...
            <p className="text-sm font-bold text-foreground mt-3 group-hover:text-foreground">{pillar.label}</p>
...
export default function AgentHrCommandPanel({
    snapshot,
    accent,
    accentMuted,
    visuals,
    ...
    const shortlist = extractCvShortlist(visuals, 8);

    return (
        <div className="space-y-5">
            {/* Workforce headline — HR language, not generic KPIs */}
            <div
            ...
            >
                ...
                {!isEmpty && (
                    <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-border/50">
                        {[
                            { label: "CVs in scope", value: snapshot.stats.cvs },
                            { label: "Ranked", value: snapshot.stats.cvsScored },
                            {
                                label: "Top score",
                                value: snapshot.stats.topCvScore != null ? `${Math.round(snapshot.stats.topCvScore)}` : "—",
                            },
                ...
                {isEmpty && (
                    ...
                    >
                        <Upload size={15} /> Upload HR documents
                    </Link>
                )}
            </div>

            {/* Four HR pillars — how HR managers organize work */}
            <section>
            ...
            {/* Priority queue — what needs action today */}
            <section className="rounded-2xl border border-border bg-surface/30 p-4">
                <div className="flex items-center justify-between gap-2 mb-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground-muted flex items-center gap-1.5">
                        <ClipboardList size={12} /> Today&apos;s priorities
                    </p>
                    <button
                        type="button"
